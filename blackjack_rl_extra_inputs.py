...
class BlackjackEnvironment:
    ...
    def reset(self) -> np.ndarray:
        ...
        # Add 3 other players
        self.game.other_players_hand = Hand()
        for i in range(0,15):
            self.game.other_players_hand.add_card(self.game.deck.deal_card())
            self.game.other_players_hand.add_card(self.game.deck.deal_card())
    ...
    def _get_state(self) -> np.ndarray:
        ...
        # get number of visible cards per value (ace as 11)
        card_counts = {i: 0 for i in range(2, 12)}
        for card in self.game.player_hand.cards:
            card_counts[card.get_value()] += 1
        for card in self.game.other_players_hand.cards:
            card_counts[card.get_value()] += 1
        ...
        # State: [player_value, dealer_visible, usable_ace, is_blackjack, card_counts]
        state = np.array([player_value, dealer_visible, int(usable_ace), int(is_blackjack)], dtype=np.float32)
        state = np.append(state, np.array(list(card_counts.values()), dtype=np.float32))
...
class DQN(nn.Module):
    ...
    def __init__(self, input_size: int = 14, hidden_size: int = 128, output_size: int = 2):
...
class BlackjackDQNAgent:
    ...
    def __init__(self, state_size: int = 14, action_size: int = 2, lr: float = 0.001):
...
def main():
    ...
    # Train the agent
    agent, episode_rewards = train_agent(episodes=30000, eval_every=500)
